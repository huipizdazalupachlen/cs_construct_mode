...
do
	local strPrefab = [==["table" {
		...
		"526"
		{
			...
			"model_player"		"models/weapons/v_knife_kukri.mdl"
			"model_world"		"models/weapons/w_knife_kukri.mdl"
			"model_dropped"		"models/weapons/w_knife_kukri_dropped.mdl"
			"used_by_classes"
			{
				"terrorists"		"1"
				"counter-terrorists"		"1"
			...
			"attributes"
			{
				"pedestal display model"		"models/weapons/v_knife_kukri_inspect.mdl"
			}
			"inventory_image_data"
			{
				"camera_angles"		"0 0 0"
	...
	for itemID, knife in next, kvPrefab do
		knife.name = knife.name:gsub("weapon_", "weapon_swcs_")
		local vm = Model(knife.model_player:gsub("weapons/", "weapons/csgo/"))
		local wm = Model(knife.model_world:gsub("weapons/", "weapons/csgo/"))
	...
	knives["weapon_swcs_knife_ghost"].SwingSound = "Player.GhostKnifeSwish"
	knives["weapon_swcs_knife_ghost"].SlashSound = "Player.GhostKnifeHit"
	knives["weapon_swcs_knife_ghost"].HitSound = "Player.GhostKnifeHit"
	knives["weapon_swcs_knife_ghost"].StabSound = "Player.GhostKnifeHit"
	if CLIENT then
		knives["weapon_swcs_knife_ghost"].SelectIcon = Material("materials/hud/swcs/select/knife_flip.png", "smooth")
	end
	...
	for classname, t in next, knives do
		local SWEP = {
			...
			Category          = "#spawnmenu.category.swcs_knives",
			IsKnife           = true,
			IsSWCSWeapon      = true,

			AutoSpawnable     = false, -- ttt
			Kind              = swcs.InTTT and WEAPON_MELEE,
			AllowDelete       = false,
			AllowDrop         = false,
			NoSights          = true,
			IsSilent          = true,
		...
		if t.SelectIcon then
			SWEP.SelectIcon = t.SelectIcon
		else
			local name = string.gsub(classname, "weapon_swcs_", "")
			local filename = Format("materials/hud/swcs/select/%s.png", name)
			if CLIENT and file.Exists(filename, "GAME") then
				SWEP.SelectIcon = Material(string.sub(filename, 11), "smooth")
			end
		end
		...
		if t.SwingSound then
			SWEP.SwingSound = t.SwingSound
		end
		if t.SlashSound then
			SWEP.SlashSound = t.SlashSound
		end
		if t.HitSound then
			SWEP.HitSound = t.HitSound
		end
		if t.StabSound then
			SWEP.StabSound = t.StabSound
		end
...
if CLIENT then
	local strEnglish = [==[ "english" {
		...
		"SFUI_WPNHUD_knife_kukri" "Kukri Knife"
	}]==]
...
do
	local strSounds = string.gsub([==["sounds"  {
		...
		"Player.GhostKnifeSwish"
		{
			...
			"rndwave"
			{
				"wave"		"~)player/csgo/halloween/ghost_swish_c_01.wav"
				"wave"		"~)player/csgo/halloween/ghost_swish_c_02.wav"
				"wave"		"~)player/csgo/halloween/ghost_swish_c_03.wav"
				"wave"		"~)player/csgo/halloween/ghost_swish_c_04.wav"
			}
		}
		...
		"Player.GhostKnifeHit"
		{
			...
			"rndwave"
			{
				"wave"		"~player/csgo/halloween/ghost_impact_c_01.wav"
				"wave"		"~player/csgo/halloween/ghost_impact_c_02.wav"
				"wave"		"~player/csgo/halloween/ghost_impact_c_03.wav"
				"wave"		"~player/csgo/halloween/ghost_impact_c_04.wav"
			}
		}
	}]==], "\\", "\\\\")
	...
	for _, kv in next, kvoSounds do
		...
		elseif isstring(t.volume) then
			...
			for _, v in ipairs(tVolume) do
				local vol = tonumber(v)
				if vol ~= nil then
					table.insert(volData, vol)
				end
			end
