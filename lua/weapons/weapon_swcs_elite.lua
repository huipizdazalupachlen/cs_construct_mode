...
SWEP.Spawnable = true
SWEP.HoldType = "duel"
SWEP.WorldModel = Model("models/weapons/csgo/w_pist_elite.mdl")
SWEP.ViewModel = Model("models/weapons/csgo/v_pist_elite.mdl")
if CLIENT then
	SWEP.SelectIcon = Material("hud/swcs/select/elite.png", "smooth")
end
...
hook.Add("DoAnimationEvent", "swcs.elites", function(ply, event, data)
	local wep = ply:GetActiveWeapon()
	if event == PLAYERANIMEVENT_ATTACK_PRIMARY and wep:IsValid() and wep.IsSWCSWeapon and wep.IsElites then
		if data == 1 then
			ply:AddVCDSequenceToGestureSlot(GESTURE_SLOT_ATTACK_AND_RELOAD, ply:LookupSequence("range_dual_r"), 0, true)
			return ACT_INVALID
		else
			ply:AddVCDSequenceToGestureSlot(GESTURE_SLOT_ATTACK_AND_RELOAD, ply:LookupSequence("range_dual_l"), 0, true)
			return ACT_INVALID
		end
	end
end)
function SWEP:DoPlayerAttackAnimation(owner)
	if owner:IsPlayer() then
		owner:DoCustomAnimEvent(PLAYERANIMEVENT_ATTACK_PRIMARY, self:FiringLeft() and 0 or 1)
	else
...
function SWEP:GetEjectBrassAttachmentIndex_3rdPerson()
	if self:FiringLeft() then
		return self:LookupAttachment("shell_eject2")
	else
