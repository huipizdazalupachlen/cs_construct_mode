...
if swcs.InTTT then
	...
	if CLIENT then
		SWEP.EquipMenuData = {
			type = "item_weapon",
			desc = "The Silenced USP Pistol has a detachable silencer\nthat gives shots less recoil while suppressing\nattention-getting noise.",
		}
	end
end
...
SWEP.WorldModel = Model("models/weapons/csgo/w_pist_223.mdl")
SWEP.ViewModel = Model("models/weapons/csgo/v_pist_223.mdl")
if CLIENT then
	SWEP.SelectIcon = Material("hud/swcs/select/usp_silencer.png", "smooth")
	SWEP.SelectIconAlt = Material("hud/swcs/select/usp_silencer_off.png", "smooth")
end
