...
if swcs.InTTT then
	...
	if CLIENT then
		SWEP.EquipMenuData = {
			type = "item_weapon",
			desc = "High risk and high reward, the infamous AWP is\nrecognizable by its signature report and one-shot,\none-kill policy.",
		}
	end
end
...
SWEP.Spawnable = true
SWEP.HoldType = "ar2"
SWEP.WorldModel = Model("models/weapons/csgo/w_snip_awp.mdl")
SWEP.ViewModel = Model("models/weapons/csgo/v_snip_awp.mdl")
if CLIENT then
	SWEP.SelectIcon = Material("hud/swcs/select/awp.png", "smooth")
end
